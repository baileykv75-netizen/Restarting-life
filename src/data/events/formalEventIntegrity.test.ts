...
describe('formal event integrity', () => {
  it('locks the V1.1 playable-slice content counts', () => {
    const catalog = createEventCatalog(FORMAL_EVENTS)
    expect(ORDINARY_EVENTS).toHaveLength(46)
    ...
    expect(FORMAL_EVENTS).toHaveLength(69)
    expect(catalog.size).toBe(69)
  ...
  it('keeps every long-term chain at three existing nodes', () => {
    ...
    for (const chain of LONG_TERM_CHAINS) {
      ...
      for (const eventId of chain.eventIds) expect(catalog.has(eventId)).toBe(true)
    }
  })
  ...
  it('keeps event metadata structurally valid', () => {
  ...
  it('gives the new playable-slice decisions explicit result narration', () => {
    const sliceEvents = FORMAL_EVENTS.filter((event) => event.id.startsWith('v11_'))
    expect(sliceEvents).toHaveLength(16)
    for (const event of sliceEvents) {
      expect(event.choices.length).toBeGreaterThanOrEqual(2)
      expect(event.choices.every((choice) => Boolean(choice.resultText))).toBe(true)
    }
  })
})
