...
export function CharacterPanel({ state, onUnequip }: CharacterPanelProps) {
  ...
  return (
    <aside className={`panel character-panel compactible-panel ${expanded ? 'expanded' : 'collapsed'}`} aria-label="人物状态">
      ...
      <div className="character-quick-facts">
        <span>灵石 <strong>{state.resources.spiritStones}</strong></span>
        <span>修为 <strong>{state.resources.cultivation}</strong></span>
      </div>
      {(injuries.length > 0 || poisons.length > 0) && <div className="character-condition-row">
        {injuries.length > 0 && <span>{injuries.some((injury) => injury.kind === 'severe') ? '重伤' : '有伤'}</span>}
        {poisons.length > 0 && <span>{poisons.some((poison) => poison.severity === 'serious') ? '重度中毒' : '中毒'}</span>}
      </div>}
      {qingyunMembership && <p className="character-sect-line">{activeQingyun ? '青云宗' : '曾属青云宗'} · {formatSectRank(qingyunMembership.rank)}</p>}
      <button className="panel-expand-button" onClick={() => setExpanded((value) => !value)} type="button">{expanded ? '收起人物详情' : '查看人物详情'}</button>
      ...
      {expanded && <div className="panel-expanded-content">
        ...
        {qingyunMembership && <div className="subsection sect-identity-subsection">
          ...
          <div className="trait-card">
            ...
            <div className="trait-effects"><span>宗门贡献 {getSectContribution(state)}</span>{(qingyunMembership.violations?.length ?? 0) > 0 && <span>违规记录 {qingyunMembership.violations!.length} 条</span>}</div>
          </div>
        </div>}
        ...
        <div className="subsection">
          ...
          <div className="trait-stack">
            {talents.map((talent) => {
              ...
              return <div className="trait-card" key={talent.id}><strong>{talent.name}</strong><p>{active?.mechanics ?? talent.description}</p>{!active && <div className="trait-effects">{effectLabels(talent.statModifiers, talent.spiritStones).map((effect) => <span key={effect}>{effect}</span>)}</div>}</div>
            })}
          </div>
        </div>
