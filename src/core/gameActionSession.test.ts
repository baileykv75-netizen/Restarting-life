...
describe('GameAction session dispatch', () => {
  it('records V2 GameAction commands in the existing debug/replay path', () => {
    ...
    expect(result.session.state.knowledge.locations.black_wind_mountain).toBe('rumored')
    expect(result.session.debugLog).toHaveLength(1)
    expect(result.session.debugLog[0].command).toEqual({
      type: 'game-action',
      action: {
        type: 'SET_LOCATION_KNOWLEDGE',
        locationId: 'black_wind_mountain',
    ...
    expect(result.session.debugLog[0].effectTypes).toEqual([
      'game-action:SET_LOCATION_KNOWLEDGE',
    ])
  ...
  it('lets the existing persistence lifecycle archive natural death from a GameAction', () => {
    ...
    const result = applyPersistentCommand(nearDeath, {
      type: 'game-action',
