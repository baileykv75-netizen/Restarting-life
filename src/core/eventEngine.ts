  ...
  for (const event of events) {
    for (const choice of event.choices) {
      if (choice.nextEventId && !catalog.has(choice.nextEventId)) {
        throw new Error(
          `Unknown nextEventId in ${event.id}/${choice.id}: ${choice.nextEventId}`,
        )
      }
      ...
      for (const effect of choice.effects) {
        if (effect.type === 'queueEvent' && !catalog.has(effect.eventId)) {
          throw new Error(
            `Unknown queueEvent target in ${event.id}/${choice.id}: ${effect.eventId}`,
          )
        }
      }
    }
...
export function drawEvent(
  ...
  const nextState = markEventActive(
...
export function getAvailableChoices(
  ...
  return event.choices.filter((choice) =>
    matchesAllConditions(state, choice.conditions),
  )
}

function activateNextQueuedEvent(
  state: GameState,
  catalog: EventCatalog,
): GameState {
  let nextState = state
  ...
  while (nextState.status === 'playing' && nextState.events.queue.length > 0) {
    ...
    const event = catalog.get(eventId)
    if (!event) {
      throw new Error(`Unknown queued event id: ${eventId}`)
    }
    ...
    if (event.once && nextState.events.history.includes(event.id)) {
      continue
    }
...
export function resolveEventChoice(
  ...
  if (!event) {
    throw new Error(`Active event is missing from catalog: ${currentEventId}`)
  }
