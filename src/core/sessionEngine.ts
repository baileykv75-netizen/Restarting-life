...
import { resolveCultivateDays, resolveCultivationInitialization, resolveMainTechniqueSelection } from './cultivationEngine'
...
import { hasActiveInjury, resolveRecuperateDays, resolveUseTreatmentItem } from './injuryEngine'
...
function actionNarrative(action: PlayerAction): string { if (action === 'cultivate') return '这一段修炼告一段落。'; if (action === 'explore') return '你结束这次外出，回到了熟悉的地方。'; if (action === 'livelihood') return '这段日子忙完了，报酬也已经结清。'; return '' }
function travelOutcome(before: GameSession['state'], result: TravelResult, fast: boolean): ResolvedOutcome {
  ...
  const elapsed = Math.max(0, result.state.worldDay - before.worldDay)
  const changes: StateChange[] = [{ label: '时间', value: `+${formatDuration(elapsed)}`, tone: 'neutral' }]
  ...
  return { title: result.arrived ? (fast ? '赶路结束' : `抵达${destination?.name ?? '目的地'}`) : '旅途中止', narrative: result.arrived ? fast ? `你沿已经走熟的路线前往${destination?.name ?? '目的地'}，共用了${formatDuration(elapsed)}。` : `你从${origin?.name ?? '出发地'}前往${destination?.name ?? '目的地'}，用了${formatDuration(elapsed)}，已经抵达。` : `这段路原本预计前往${destination?.name ?? '目的地'}，但你没能走到终点。`, changes, consequence: null }
}
...
export function executeSessionCommand(session: GameSession, command: SessionCommand): SessionCommandResult {
  ...
  if (command.type === 'childhood-choice') {
    ...
    if (before.events.currentEventId !== null || pendingAction !== null) return { session: workingSession, applied: false, reason: 'EVENT_PENDING' }
    if (command.action === 'enter' && hasActiveInjury(before, 'severe')) return { session: workingSession, applied: false, reason: 'SEVERE_INJURY_BLOCKS_SECRET_REALM' }
    if (command.action === 'enter' && hasSeriousPoison(before)) return { session: workingSession, applied: false, reason: 'SERIOUS_POISON_BLOCKS_SECRET_REALM' }
  ...
  } else if (command.type === 'apply-poison-condition') {
    if (before.events.currentEventId !== null || pendingAction !== null) return { session: workingSession, applied: false, reason: 'EVENT_PENDING' }
    const result = resolveApplyPoisonCondition(before, command.family); nextState = result.state; applied = result.applied; reason = result.reason; pendingAction = null; effectTypes = ['health:apply-poison']
  } else if (command.type === 'use-treatment-item') {
    if (before.events.currentEventId !== null || pendingAction !== null) return { session: workingSession, applied: false, reason: 'EVENT_PENDING' }
    const result = resolveUseTreatmentItem(before, command.itemId, command.injuryId); nextState = result.state; applied = result.applied; reason = result.reason; pendingAction = null
    if (applied && result.completed && nextState.status === 'playing') pendingResult = result.outcome ?? null
    effectTypes = ['health:treatment']
