...
describe('GameAction reducer', () => {
  it('applies the minimal V2 state actions immutably', () => {
    ...
    const located = applyGameAction(stage.state, {
      type: 'SET_CURRENT_LOCATION',
      locationId: 'qingxia_market',
    })
    expect(located.state.world.currentLocationId).toBe('qingxia_market')

    const rumored = applyGameAction(located.state, {
      type: 'SET_LOCATION_KNOWLEDGE',
      locationId: 'black_wind_mountain',
      status: 'rumored',
    })
    expect(rumored.state.knowledge.locations.black_wind_mountain).toBe('rumored')

    const discovered = applyGameAction(rumored.state, {
      type: 'SET_LOCATION_KNOWLEDGE',
      locationId: 'black_wind_mountain',
      status: 'discovered',
    })
    expect(discovered.state.knowledge.locations.black_wind_mountain).toBe('discovered')
  ...
  it('rejects invalid inputs without partially modifying state', () => {
  ...
  it('never downgrades discovered location knowledge back to rumored', () => {
    ...
    const discovered = applyGameAction(initial, {
      type: 'SET_LOCATION_KNOWLEDGE',
      locationId: 'hidden_cave',
      status: 'discovered',
    })
    const downgraded = applyGameAction(discovered.state, {
      type: 'SET_LOCATION_KNOWLEDGE',
      locationId: 'hidden_cave',
      status: 'rumored',
    })
    ...
    expect(downgraded.state.knowledge.locations.hidden_cave).toBe('discovered')
  })
  ...
  it('advances the one world clock and preserves natural death resolution', () => {
    ...
    const nearDeath = {
      ...initial,
      worldDay: initial.identity.birthDay + 80 * DAYS_PER_YEAR - 1,
    }
