...
describe('Chronicle V2', () => {
  it('records what happened, what the player chose, and the exact consequence', () => {
    ...
    expect(entry?.title).toBe('山泉歇脚')
    expect(entry?.sceneText).toContain('清泉')
    expect(entry?.choiceText).toBe('收起碎灵石')
  ...
  it('keeps all formal event definitions valid after adding chronicle state', () => {
