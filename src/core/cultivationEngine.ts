...
import { getCultivationTechniqueById, getTechniqueById, techniqueSupportsRealm, type CultivationTechniqueDefinition } from '../data/techniques'
...
export type CultivationBlockReason = 'GAME_ENDED' | 'NOT_A_CULTIVATOR' | 'REALM_COMPLETE'
...
export function getEffectiveSpiritRootMultiplier(state: GameState): number {
  ...
  if (typeof reformedMultiplier === 'number' && Number.isFinite(reformedMultiplier) && reformedMultiplier > 0) return reformedMultiplier
  return 0
}
...
export function applyAutomaticStageProgression(state: GameState): GameState {
  ...
  if (state.cultivation.realm === 'qi') {
    while (stage < 9 && cultivation >= QI_LAYER_THRESHOLD) { cultivation -= QI_LAYER_THRESHOLD; stage += 1 }
  } else if (state.cultivation.realm === 'foundation') {
    if (stage === 1 && cultivation >= FOUNDATION_EARLY_TO_MIDDLE_THRESHOLD) { cultivation -= FOUNDATION_EARLY_TO_MIDDLE_THRESHOLD; stage = 2 }
    if (stage === 2 && cultivation >= FOUNDATION_MIDDLE_TO_LATE_THRESHOLD) { cultivation -= FOUNDATION_MIDDLE_TO_LATE_THRESHOLD; stage = 3 }
  }
  if (stage === state.cultivation.stage && cultivation === state.resources.cultivation) return state
  return { ...state, cultivation: { ...state.cultivation, stage }, resources: { ...state.resources, cultivation } }
}
...
/** R16 formal V2 cultivation, extended in R19 for foundation. */
...
const METHOD_SEED_TO_TECHNIQUE: Readonly<Record<string, string>> = {
  xiaozhoutian_tuna: 'xiaozhoutian_tuna', qingyuan_yinqi: 'qingyuan_yinqi', xie_basic_qi_method: 'xiaozhoutian_tuna', lu_basic_qi_method: 'xiaozhoutian_tuna',
}
const ENVIRONMENT_MULTIPLIER: Record<QiDensity, number> = { none: 0.55, thin: 0.7, low: 0.8, medium: 1, high: 1.15 }
const ENVIRONMENT_LABEL: Record<QiDensity, string> = { none: '几乎无灵气', thin: '灵气稀薄', low: '灵气偏低', medium: '灵气普通', high: '灵气充沛' }

export interface CultivationFactor { label: string; multiplier: number }
export interface CultivationPreview { days: CultivationDuration; gain: number; technique: CultivationTechniqueDefinition; factors: CultivationFactor[]; environmentLabel: string }
export interface R16CultivationResult { state: GameState; applied: boolean; completed: boolean; reason?: string; gainApplied: number; enteredQi: boolean; outcome?: ResolvedOutcome }

function r16Rejected(state: GameState, reason: string): R16CultivationResult { return { state, applied: false, completed: false, reason, gainApplied: 0, enteredQi: false } }
function isCultivationDuration(days: number): days is CultivationDuration { return CULTIVATION_DURATIONS.includes(days as CultivationDuration) }
function getMethodSeed(state: GameState): string | null { const flag = state.flags.cultivation_method_access_seed; if (typeof flag === 'string' && flag.length > 0) return flag; const tag = state.tags.find((entry) => entry.startsWith('cultivation_method_access:')); return tag ? tag.slice('cultivation_method_access:'.length) : null }
...
export function resolveCultivationInitialization(state: GameState): R16CultivationResult {
  ...
  return { state: { ...state, cultivation: { ...state.cultivation, practiceInitialized: true, knownTechniqueIds: getInitialTechniqueIds(state), mainTechniqueId: null } }, applied: true, completed: true, gainApplied: 0, enteredQi: false }
}
...
export function resolveMainTechniqueSelection(state: GameState, techniqueId: string): R16CultivationResult {
  ...
  const cultivationTechnique = getCultivationTechniqueById(techniqueId)
  if (!cultivationTechnique) return r16Rejected(state, 'TECHNIQUE_BALANCE_NOT_FROZEN')
...
function getEnvironment(state: GameState): { multiplier: number; label: string } | null {
  ...
  if (location.id === 'qingyun_sect' && location.qiDensity === 'high' && state.identity.faction !== 'qingyun') return { multiplier: ENVIRONMENT_MULTIPLIER.medium, label: '宗门外围 · 灵气普通' }
  return { multiplier: ENVIRONMENT_MULTIPLIER[location.qiDensity], label: ENVIRONMENT_LABEL[location.qiDensity] }
}
function getAffinityMultiplier(state: GameState, technique: CultivationTechniqueDefinition): number { if (technique.universal) return 1; const root = SPIRIT_ROOTS.find((entry) => entry.id === state.identity.spiritRootId); if (!root) return 0.85; return root.elements.some((element) => technique.preferredElements.includes(element)) ? 1.15 : 0.85 }
function getTraitMultiplier(state: GameState, technique: CultivationTechniqueDefinition, days: CultivationDuration): { multiplier: number; factors: CultivationFactor[] } {
  let multiplier = 1; const factors: CultivationFactor[] = []
  if (days >= 10 && state.identity.talentIds.includes('still_mind')) { multiplier *= 1.08; factors.push({ label: '静心守一（长修炼）', multiplier: 1.08 }) }
  if (technique.id === 'chiyang_jue' && state.identity.physiqueIds.includes('red_yang_body')) { multiplier *= 1.1; factors.push({ label: '赤阳灵体契合', multiplier: 1.1 }) }
  if (technique.id === 'hanshui_jing' && state.identity.physiqueIds.includes('mysterious_yin_body')) { multiplier *= 1.1; factors.push({ label: '玄阴灵体契合', multiplier: 1.1 }) }
...
export function formatCultivationRealm(realm: Realm, stage: number): string {
  ...
  if (realm === 'foundation') return `筑基${stage === 1 ? '前期' : stage === 2 ? '中期' : stage === 3 ? '后期' : '圆满'}`
  return '金丹'
}
export function formatCultivationProgress(points: number): string { return `${(Math.max(0, Math.min(CULTIVATION_STAGE_POINTS, points)) / 10).toFixed(1)}%` }
export function isQiNineComplete(state: GameState): boolean { return state.cultivation.realm === 'qi' && state.cultivation.stage === 9 && state.resources.cultivation >= CULTIVATION_STAGE_POINTS }
export function isFoundationComplete(state: GameState): boolean { return state.cultivation.realm === 'foundation' && state.cultivation.stage === 4 && state.resources.cultivation >= CULTIVATION_STAGE_POINTS }

function applyR16Progress(state: GameState, gain: number): { state: GameState; enteredQi: boolean } {
  let realm = state.cultivation.realm; let stage = state.cultivation.stage; let points = state.resources.cultivation + gain; let enteredQi = false
  if (realm === 'mortal' && points >= CULTIVATION_STAGE_POINTS) { points -= CULTIVATION_STAGE_POINTS; realm = 'qi'; stage = 1; enteredQi = true }
  if (realm === 'qi') { while (stage < 9 && points >= CULTIVATION_STAGE_POINTS) { points -= CULTIVATION_STAGE_POINTS; stage += 1 }; if (stage === 9) points = Math.min(points, CULTIVATION_STAGE_POINTS) }
  if (realm === 'foundation') { while (stage < 4 && points >= CULTIVATION_STAGE_POINTS) { points -= CULTIVATION_STAGE_POINTS; stage += 1 }; if (stage === 4) points = Math.min(points, CULTIVATION_STAGE_POINTS) }
  let next: GameState = { ...state, resources: { ...state.resources, cultivation: points }, cultivation: { ...state.cultivation, realm, stage } }
  if (enteredQi) next = { ...next, chronicle: [...next.chronicle, { id: `${next.runId}:cultivation:entered-qi:${next.worldDay}`, startDay: next.worldDay, endDay: next.worldDay, title: '引气入体', sceneText: '灵气第一次真正沿着周天在体内运转。', narrative: '你完成了第一次稳定周天，正式踏入炼气一层。', changes: [{ label: '境界', value: '凡人 → 炼气一层', tone: 'positive' }], importance: 'notable', sourceType: 'activity', sourceId: 'cultivation-entered-qi' }] }
...
function buildR16Outcome(before: GameState, after: GameState, preview: CultivationPreview, proficiencyBefore?: string, proficiencyAfter?: string): ResolvedOutcome {
  ...
  if (proficiencyBefore && proficiencyAfter && proficiencyBefore !== proficiencyAfter) changes.push({ label: '主修熟练', value: `${proficiencyBefore} → ${proficiencyAfter}`, tone: 'positive' })
  return { title: `闭关${formatDuration(preview.days)}`, narrative: `你按${preview.technique.name}运转周天，${formatDuration(preview.days)}后结束吐纳。`, changes,
    consequence: isQiNineComplete(after) ? '炼气九层已经圆满。继续提升需要准备筑基。' : isFoundationComplete(after) ? '筑基圆满。普通修炼已经停止，接下来需要准备结丹。' : null }
}
...
export function resolveCultivateDays(state: GameState, days: number): R16CultivationResult {
  ...
  const technique = getCultivationTechniqueById(mainTechniqueId)
  if (!technique) return r16Rejected(state, 'CULTIVATION_CONTEXT_INVALID')
  if (!techniqueSupportsRealm(technique, state.cultivation.realm)) return r16Rejected(state, 'MAIN_TECHNIQUE_REALM_UNSUPPORTED')
  ...
  if (advanced.status !== 'playing') return { state: advanced, applied: true, completed: false, gainApplied: 0, enteredQi: false }
  const progressed = applyR16Progress(advanced, preview.gain)
  const proficiencyBefore = state.cultivation.techniqueSystemInitialized ? getProficiencyLabel(getTechniqueProficiencyStage(state, mainTechniqueId)) : undefined
  const withProficiency = state.cultivation.techniqueSystemInitialized ? addTechniqueProficiency(progressed.state, mainTechniqueId, calculateTechniqueProficiencyGain(state, days)) : progressed.state
  const proficiencyAfter = state.cultivation.techniqueSystemInitialized ? getProficiencyLabel(getTechniqueProficiencyStage(withProficiency, mainTechniqueId)) : undefined
  return { state: withProficiency, applied: true, completed: true, gainApplied: preview.gain, enteredQi: progressed.enteredQi, outcome: buildR16Outcome(state, withProficiency, preview, proficiencyBefore, proficiencyAfter) }
}
