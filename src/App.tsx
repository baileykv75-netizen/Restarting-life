...
function readInitialGame(): InitialViewState {
  try { return { game: loadGame(window.localStorage), error: null } }
  catch (error) { return { game: createEmptyPersistentGame(), error: error instanceof Error ? error.message : '本地存档无法读取' } }
}

function ResultPanel({ result, onContinue }: { result: ResolvedOutcome; onContinue: () => void }) {
  return (
    <section className="story-card result-card">
      <p className="story-kicker">结果</p><h2>{result.title}</h2>
      {result.narrative && <p className="story-text result-narrative">{result.narrative}</p>}
      {result.changes.length > 0 && <><div className="result-divider" /><p className="subsection-title">变化</p><div className="result-changes">{result.changes.map((change, index) => <div className={`result-change ${change.tone}`} key={`${change.label}-${index}`}><span>{change.label}</span><strong>{change.value}</strong></div>)}</div></>}
...
function App() {
  ...
  function persistStart() {
    try { const next = startAndSaveRun(window.localStorage, game, Date.now()); setGame(next); setError(null); setNotice(null) }
    catch (caught) { setNotice(caught instanceof Error ? caught.message : '无法开启新的人生') }
  }

  function persistBirthChoice(candidateId: string) {
    try { const next = chooseBirthAndSave(window.localStorage, game, candidateId); setGame(next); setNotice(null) }
    catch (caught) { setNotice(caught instanceof Error ? caught.message : '无法确定这一世') }
  }
  ...
  function recoverSave() {
    try { const next = clearGame(window.localStorage); setGame(next); setError(null); setNotice(null) }
    catch (caught) { setNotice(caught instanceof Error ? caught.message : '无法清除本地存档') }
  }

  if (error) {
    return <main className="landing-shell"><section className="landing-card danger-card"><p className="eyebrow">此世问长生 · V2.0</p><h1>本地存档需要处理</h1><p className="story-text">这份存档没有通过完整性校验。为避免继续损坏记录，游戏没有加载它。</p><p className="error-text">{error}</p><button className="primary-button" onClick={recoverSave} type="button">清除损坏存档并重新开始</button>{notice && <p className="notice">{notice}</p>}</section></main>
  }

  if (!session || !state) {
    if (game.phase === 'birth-selection' && game.pendingBirthSelection) {
      return <><BirthSelectionPanel pending={game.pendingBirthSelection} archiveCount={game.archives.length} onChoose={persistBirthChoice} onOpenArchive={() => setArchiveOpen(true)} />{notice && <p className="birth-floating-notice notice">{notice}</p>}{archiveOpen && <ArchivePanel records={game.archives} onClose={() => setArchiveOpen(false)} />}</>
    }
    return (
      <main className="landing-shell">
        <section className="landing-card"><p className="eyebrow">此世问长生 · V2.0</p><h1>此世问长生</h1><p className="landing-lead">每一次开始，都是另一个人的一生。</p><p className="muted">有人生来近仙，有人一辈子也未必摸得到那道门。</p><div className="landing-actions"><button className="primary-button" onClick={persistStart} type="button">开始一段人生</button><button className="secondary-button" onClick={() => setArchiveOpen(true)} type="button">人生档案 · {game.archives.length}</button></div>{notice && <p className="notice">{notice}</p>}</section>
  ...
  return (
    <main className="game-shell">
      <header className="topbar app-header"><div className="shell-brand"><p className="eyebrow">此世问长生 · V2.0</p><h1>此世问长生</h1></div><div className="topbar-actions"><button className="text-button" onClick={() => setArchiveOpen(true)} type="button">人生档案 {game.archives.length}</button></div></header>
      ...
      <div className="game-grid">
        ...
        <section className="main-stage" aria-label="当前经历">
          {state.lifeStage === 'childhood' ? (
            <section className="story-card childhood-entry-card"><p className="story-kicker">童年</p><h2>这一世已经开始。</h2><p className="story-text">你还只是个孩子。出身、资质和最初的人际关系已经确定，接下来发生的事会从你真正生活的地方开始。</p></section>
            ...
            <><ActionPanel state={state} actions={getAvailableActions(state) as PlayerAction[]} onAction={(action) => persistCommand({ type: 'action', action })} /></>
          )}
          {notice && <p className="notice">{notice}</p>}
        </section>
        <ChroniclePanel entries={state.chronicle} birthDay={state.identity.birthDay} />
      </div>
      {archiveOpen && <ArchivePanel records={game.archives} onClose={() => setArchiveOpen(false)} />}
    </main>
  )
